...
In this phase, we are researching possible topics, reading papers, and formulating a proposal for our research topic. Due date: October 5th 2020
\subsection{Research Phase}
In this phase, we are researching the approved topic by reading papers and starting to plan our experimenting phase. We are also researching what we should be using to perform our experimenting phase along with what metrics should be measured to indicate a successful experiment.
\subsection{Experimenting Phase}
In this phase, we are experimenting with the products we found during our research and attempting to generate metrics.
\subsection{Reporting Phase}
In this phase, we are analyzing, exploiting, and reporting on the data from the previous phase. 
\subsection{Presentation Phase}
In this phase, we are preparing the the reported data in a presentable format.
\section{Final Outcome}
We hope to have a framework that offers security as the preliminary design consideration along with support for "drop in" solution legacy applications.
...
\item Latency
\item Throughput
\item \# Encrypted packets
\item \# Of clear text packets
\item Resource utilization of host
\item Other?
\end{itemize}
\subsection{Licensing}
All software used is free to use, open source, or licensed under GPL/GNU.
\subsection{Software that will be used}
\begin{itemize}
\item Kibana
\item Elasticsearch
\item Docker Engine
\item Docker Compose
\item Libvirt - QEMU/KVM
\item Shadowsocks
\item STunnel
\item Wireguard
\item Consul
...
\end{itemize}
\subsection{Setup}
We have planned to have a lab environment that is consistent and easy to use. We will be using Docker Engine with Docker Compose scripts to allow for a easy lab setup. A Github repository will be provided with all our research and scripts.

We have two options where we will try to run our lab environment. The first option is on our own laptops, which is the preferred option. With the few containers that we are expected to have running at once, we should have no issues. The second option is in Georges' home lab, which has a lot more computing power. 
